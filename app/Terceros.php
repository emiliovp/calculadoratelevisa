...
class Terceros extends Model
{
    ...
    public function tercerosAsignados($noBadge = null) {
        return Terceros::select(
            ...
            'tcs_external_employees.created_at',
            'tcs_external_employees.status'
        )
        ...
        ->where([
            'tcs_autorizador_responsable.number' => Auth::user()->noEmployee,
            'tcs_autorizador_responsable.type' => 1,
            'tcs_autorizador_responsable.status' => 1,
            'tcs_external_employees.status' => 1
        ])
        ->orWhere([
            'tcs_autorizador_responsable.type' => 2
        ])
        ->where([
            'tcs_autorizador_responsable.number' => Auth::user()->noEmployee,
            'tcs_autorizador_responsable.status' => 1,
