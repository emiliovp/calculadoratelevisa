...
class FUSSysadminWtl extends Model
{
    protected $table = "fus_sysadmin_wtl";
    ...
    {
       $a = FUSSysadminWtl::select(['id as folio_fus',
    ...
    {
       $a= FUSSysadminWtl::from('fus_sysadmin_wtl')
        ->select([
    ...
    public function getFusById($id) {
        return FUSSysadminWtl::find($id)->toArray();
    }
    public function getFusByIdWtl($id) {
        return FUSSysadminWtl::where('estado_fus', '=', 1)
        ->whereIn('tipo_fus', [1,2,3])
        ->where('id', '=', $id)
        ->first();
    }
    public function getFusByIdWtlReenvio($id) {
        return FUSSysadminWtl::where('id', '=', $id)
    ...
    {
        FUSSysadminWtl::find($id)->update(['fus_cuerpo' => json_encode($data, JSON_PRETTY_PRINT)]); // update device 1
    }
    ...
    public function getFusInfProcesada($id) {
        ...
        $fus = FUSSysadminWtl::find($id)->toArray();
    ...
    public function changeStatusApp($idfus, $idapp, $fusConfiguracionAutorizacionesId) {
        ...
        foreach($relConfig AS $rows) {
            $query = FUSSysadminWtl::from('fus_sysadmin_wtl AS fsw')
            ->select([
                DB::raw('
                    (
                        SELECT 
    ...
    public function reporteFus(){
        return FUSSysadminWtl::select('id AS Folio',  
    ...
    public function cambioEstadoFusApp($idFus, $estado) {
        FUSSysadminWtl::find($idFus)->update(['estado_fus' => $estado, 'fecha_atencion' => DB::raw('now()')]);
    }
}