    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{route('updateoptcat')}}">
                        @csrf
                        ...
                        <div class="form-group row">
                            <div class="col-md-12">
                                ...
                                <input type="text" placeholder="Descripción de la opción..." class="form-control{{ $errors->has('cat_op_descripcion') ? ' is-invalid' : '' }}" value="{{ !empty(old('cat_op_descripcion')) ? old('cat_op_descripcion') : $opcionAEditar['cat_op_descripcion'] }}" name="cat_op_descripcion" required id="cat_op_descripcion"/>
                                ...
                                <label for="cat_id" class="txt-bold">Dependencia con catálogo</label>
                                    ...
                                    @foreach($catalogos AS $row)
                                        ...
                                        @if($row['id'] == $padre)
                                            ...
                                            @endphp
                                        @endif
                                        <option {{$selected}} value="{{$row['id']}}">{{$row["cat_nombre"]}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-1">
                                <input type="hidden" name="hiddenAutorizaciones" id="hiddenAutorizaciones" value="{{$opcionAEditar['id_rel_aut']}}" />
...
@push('scripts')
    <script>
            ...
            data:{cat: cat},
                success: function(response) {
                    $.each(response,function(index,value){
                        ...
                        $('#cat_opciones_id').append("<option "+select+" value='"+value.id+"'>"+value.cat_op_descripcion+"</option>");
        ...
        function cargarLista() {
            var tipo = 'autorizaciones';
            var hiddenIdTipo = '#hiddenAutorizaciones';
            var selectSelectedIdTipo = '#autorizaciones option';
            var divUlIdTipo = '#content_list_autorizacionesadds';
            var ulIdTipo = '#list_autorizaciones';
            var titleList = 'Lista de autorizaciones agregadas';
